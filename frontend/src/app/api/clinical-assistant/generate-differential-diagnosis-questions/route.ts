...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${apiUrl}/clinical/generate-differential-diagnosis-questions`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { detail: errorData.detail || errorData || `Backend error: ${response.status}` },
    ...
    console.error('Error in differential diagnosis questions API route:', error);
    return NextResponse.json(
