...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!userRoleFromToken) {
      console.error('API Route /api/clinical/generate-lab-insights-translated: User ' + userId + ' has no role in token. publicMetadata:', JSON.stringify(sessionClaims?.publicMetadata));
    ...
    try {
      ...
      console.error("API Route /api/clinical/generate-lab-insights-translated: Invalid JSON payload.", error);
    ...
    if (!apiPayload.lab_results || !apiPayload.user_role) {
        console.error("API Route /api/clinical/generate-lab-insights-translated: Missing lab_results or user_role in payload.");
    ...
    if (!Object.values(UserRoleForLLM).includes(apiPayload.user_role)) {
        console.error("API Route /api/clinical/generate-lab-insights-translated: Invalid user_role in payload.");
        return NextResponse.json({ error: 'Invalid user_role in payload.' }, { status: 400 });
    }

    console.log('API Route /api/clinical/generate-lab-insights-translated: Received payload for user ' + userId + ' (Role: ' + userRoleFromToken + '):', JSON.stringify(apiPayload, null, 2));
    ...
    if (!fastApiResponse.ok) {
      ...
      console.error(`API Route /api/clinical/generate-lab-insights-translated: FastAPI request failed with status ${fastApiResponse.status}:`, errorBody);
    ...
    console.log('API Route /api/clinical/generate-lab-insights-translated: FastAPI response received and parsed.');

    return NextResponse.json(insightsToReturn, { status: 200 });

  } catch (error: any) {
    console.error("Error in API Route /api/clinical/generate-lab-insights-translated:", error.message, error.stack);
