...
export async function POST(request: NextRequest) {
  try {
    ...
    // Forward to backend
    const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/groups/invitations/accept`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      return new NextResponse(JSON.stringify({
        error: errorData.error || `Failed to accept group invitation: ${response.status} ${response.statusText}`
      }), {
    ...
    console.error('Error in POST /api/groups/invitations/accept:', error);
