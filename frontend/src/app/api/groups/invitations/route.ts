  ...
  try {
    ...
    switch (action) {
      case 'accept':
        ...
        break;
      case 'decline':
        endpoint = '/api/groups/invitations/decline';
        break;
      default:
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      return new NextResponse(JSON.stringify({
        error: errorData.error || `Failed to process invitation action: ${response.status} ${response.statusText}`
      }), {
    ...
    console.error('Error in POST /api/groups/invitations:', error);
