...
// URL base da API
export const API_BASE_URL = process.env.NEXT_PUBLIC_API_URL || (process.env.NODE_ENV === 'production' ? '/api' : 'http://backend:8000');
...
export const getAPIUrl = () => {
  if (typeof window === 'undefined') { // Server-side
    ...
    const serverUrl = process.env.INTERNAL_API_URL || 'http://backend:8000';
