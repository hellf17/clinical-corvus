...
export const FileUploadComponent: React.FC<FileUploadComponentProps> = ({ 
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const apiUrl = `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/analyze/upload/${patientId}`;
  ...
  const handleGuestUpload = async () => {
    ...
    try {
      const apiUrl = `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/lab-analysis/guest`;
