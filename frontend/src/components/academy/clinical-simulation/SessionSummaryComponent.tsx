...
export interface SessionSummaryData {
  ...
  keyLearnings: string[];
  areasForFurtherPractice: string[];
  nextSteps: string[];
}
      ...
      <Accordion type="single" className="w-full">
        <AccordionItem value="item-1">
          <AccordionTrigger className="flex items-center justify-between w-full px-4 py-2 text-left font-medium text-white bg-gradient-to-r from-blue-500 to-purple-600 rounded-md shadow-md hover:from-blue-600 hover:to-purple-700 transition-all duration-300 ease-in-out">
            <span>Dr. Corvus's Session Summary</span>
          ...
          <AccordionContent className="p-4 bg-white border border-t-0 border-gray-200 rounded-b-md shadow-sm">
            <div className="space-y-3">
              <p className="text-gray-700"><strong>Overall Performance:</strong> {feedback.overallPerformance}</p>
              {feedback.keyLearnings.length > 0 && (
                <div>
                  <p className="font-semibold text-green-700">Key Learnings:</p>
                  <ul className="list-disc list-inside text-gray-700">
                    {feedback.keyLearnings.map((item, index) => (
                      <li key={index}>{item}</li>
                    ))}
                  </ul>
                </div>
              )}
              {feedback.areasForFurtherPractice.length > 0 && (
                <div>
                  <p className="font-semibold text-yellow-700">Areas for Further Practice:</p>
                  <ul className="list-disc list-inside text-gray-700">
                    {feedback.areasForFurtherPractice.map((item, index) => (
                      <li key={index}>{item}</li>
                    ))}
                  </ul>
                </div>
