...
def optimize_query_for_academic_database(query: str, database_key: str, include_recent_years: bool = True) -> str:
    ...
    # Build enhanced query based on database type            
    if database_key == "cochrane":
        # For Cochrane, emphasize systematic reviews and meta-analyses
        enhanced_query = f"site:cochranelibrary.com ({query}) (systematic review OR meta-analysis OR intervention)"
        
    elif database_key in ["sciencedirect", "bmj", "nejm", "thelancet", "jama"]:
        # For medical journals, focus on research articles
        enhanced_query = f"site:{db_config['site']} ({query}) (research OR clinical OR study OR trial)"
        
    elif database_key == "medrxiv":
        # For preprints, include preprint-specific terms
        enhanced_query = f"site:medrxiv.org ({query}) preprint"
        
    elif database_key == "ncbi_bookshelf":
        # For NCBI books, focus on textbooks and guidelines
        enhanced_query = f"site:ncbi.nlm.nih.gov/books ({query}) (statpearls OR guidelines OR textbook OR reference)"
        
    elif database_key == "ncbi_pmc":
        # For PMC, focus on free full-text articles
        enhanced_query = f"site:ncbi.nlm.nih.gov/pmc ({query}) free full text"
        
    else:
        # Default format
        enhanced_query = db_config["query_format"].format(
            query=query, 
            keywords=" OR ".join(medical_keywords[:3])  # Top 3 keywords
        )
