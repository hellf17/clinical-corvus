...
class TestFilesModule:
    """Testes para o módulo de processamento de arquivos PDF."""
    ...
    @patch("routers.files.extrair_id")
    @patch("routers.files.extrair_campos_pagina")
    async def test_process_pdf_file_function(self, mock_extrair_campos, mock_extrair_id, mock_pdf_file):
        """Testa a função de processamento de arquivos PDF."""
        # Mock das funções de extração
        mock_extrair_id.return_value = ("Paciente Teste", "10/05/2023", "14:30")
        mock_extrair_campos.return_value = [
            {
        ...
        # Testa a função
        with patch("database.models.TestCategory") as mock_category, \
             patch("database.models.LabResult") as mock_lab_result:
            
            # Configura mocks
            mock_category_instance = MagicMock()
            mock_category_instance.category_id = 1
            mock_category.return_value = mock_category_instance
            mock_db.query.return_value.filter.return_value.first.return_value = mock_category_instance
            
            # Executa a função async corretamente com await
            await process_pdf_file(mock_pdf_file, 1, 1, mock_db)
