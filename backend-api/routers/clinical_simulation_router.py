...
async def initialize_simulation(payload: InitializeSimulationRequest):
    ...
    try:
        # Create a fresh session state, ensuring all list fields are initialized as empty lists
        session_state = SessionState(
            case_context=payload.case_context,
            student_ddx=[],
            student_probe_questions=[],
            feedback_history=[]
        )
        # Ensure case_context list fields are also initialized as empty lists if they are None
        if session_state.case_context.expected_differentials is None:
            session_state.case_context.expected_differentials = []
        if session_state.case_context.learning_objectives is None:
            session_state.case_context.learning_objectives = []
...
async def execute_snapps_step_translated(payload: SNAPPSStepRequest, target_lang: str = "PT"):
    """Executes a SNAPPS step and translates the feedback into the target language."""
    logger.info(f"Received request for translated SNAPPS step. Current step: {payload.current_step}, Target Lang: {target_lang}")
    try:
        # Reuse the main logic by calling the non-translated endpoint function
        response = await execute_snapps_step(payload)
        logger.info(f"Successfully executed non-translated SNAPPS step. Response: {response.feedback}")
        ...
        logger.info(f"Translated feedback: {translated_feedback}")
        ...
        if isinstance(translated_feedback, dict) and 'recommended_questions' in translated_feedback:
            ...
            logger.info(f"Normalized recommended_questions: {translated_feedback['recommended_questions']}")
        ...
        logger.error(f"HTTPException in translated SNAPPS step: {e.detail}", exc_info=True)
        raise e # Re-raise HTTP exceptions from the original function
    except Exception as e:
        logger.error(f"Unhandled exception in translated SNAPPS step for {payload.current_step}: {str(e)}", exc_info=True)
...
async def _translate_field(field: Any, target_lang: str, field_name: str = None) -> Any:
    ...
    """
    logger.debug(f"Attempting to translate field: {field_name} with value: {field}")
    try:
        if isinstance(field, str):
            ...
            if not field.strip() or field.isupper() or field.isnumeric() or len(field) < 2:
                logger.debug(f"Skipping translation for non-translatable string: {field}")
                return field
            translated_str = await translate_with_fallback(field, target_lang, field_name)
            logger.debug(f"Translated '{field}' to '{translated_str}'")
            return translated_str
        elif isinstance(field, list):
            translated_list = [await _translate_field(item, target_lang, f"{field_name}[{i}]") for i, item in enumerate(field)]
            logger.debug(f"Translated list: {translated_list}")
            return translated_list
        elif isinstance(field, dict):
            translated_dict = {k: await _translate_field(v, target_lang, f"{field_name}.{k}") for k, v in field.items()}
            logger.debug(f"Translated dict: {translated_dict}")
        ...
        logger.debug(f"No translation needed for type: {type(field)}")
        return field
    except Exception as e:
        logger.error(f"Translation failed for field '{field_name}': {e}", exc_info=True)
...
async def _translate_field_batched(field, target_lang="PT", field_name=None):
    ...
    """
    logger.info(f"Starting _translate_field_batched for field: {field_name}")
    ...
    def collect_strings(obj, path=""):
        ...
        if isinstance(obj, str):
            ...
            if obj.strip() and not obj.isupper() and not obj.isnumeric() and len(obj) >= 2:
                ...
                paths.append(path)
                logger.debug(f"Collected string for translation: '{obj}' at path: '{path}'")
            else:
                logger.debug(f"Skipping collection of non-translatable string: '{obj}' at path: '{path}'")
            ...
            logger.debug(f"Processing Pydantic model for collection at path: {path}")
            collect_strings(obj.model_dump(), path)
        else:
            logger.debug(f"Skipping collection for non-string, non-list, non-dict, non-pydantic type: {type(obj)} at path: {path}")
    ...
    if not strings_to_translate:
        logger.info(f"No strings to translate for field: {field_name}. Returning original field.")
        return field
    ...
    try:
        ...
        logger.info(f"Batch translation completed. First few translated strings: {translated_strings[:3]}")
        ...
        logger.debug(f"Translations map created. Example: {list(translations.items())[:3]}")
        ...
        def replace_strings(obj, path=""):
            ...
            if isinstance(obj, str):
                ...
                if obj.strip() and not obj.isupper() and not obj.isnumeric() and len(obj) >= 2 and path in translations:
                    translated_value = translations[path]
                    logger.debug(f"Replacing '{obj}' at path '{path}' with '{translated_value}'")
                    return translated_value
                logger.debug(f"Keeping original string: '{obj}' at path: '{path}'")
                return obj
            elif isinstance(obj, list):
                ...
                logger.debug(f"Processing Pydantic model for replacement at path: {path}")
                field_dict = obj.model_dump()
                translated_dict = replace_strings(field_dict, path)
                return type(obj)(**translated_dict)
            logger.debug(f"Keeping original value for type: {type(obj)} at path: {path}")
            return obj
        
        # Replace all strings with their translations
        final_translated_field = replace_strings(field, field_name or "")
        logger.info(f"Finished _translate_field_batched for field: {field_name}")
        return final_translated_field
    except Exception as e:
        logger.error(f"Batch translation failed for {field_name or 'unknown'}: {e}", exc_info=True)
